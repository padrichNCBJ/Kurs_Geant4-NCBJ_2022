...
void DetectorConstruction::BuildMaterials()
{
  ...
  fWoda = nist->FindOrBuildMaterial("G4_WATER");
  ...
  G4int nComponents = 4;
  fRW3 = new G4Material("RW3", RW3_density, nComponents, kStateSolid);
  fRW3->AddElement(Hydrogen, 0.0759);
  fRW3->AddElement(Carbon, 0.9041);
  fRW3->AddElement(Oxygen, 0.008);
  fRW3->AddElement(Titanium, 0.012);
  ...
  fAirLowPressure = new G4Material("Air_low_pressure", density, Air_STD, kStateGas, temperature, pressure);
  ...
  fSoilMaterial = new G4Material("Soil",Soil_density, Soil_Ncomponents);
  fSoilMaterial->AddMaterial(O, fractionmass=O_wfrac);
  fSoilMaterial->AddMaterial(Si,fractionmass=Si_wfrac);
  fSoilMaterial->AddMaterial(Al,fractionmass=Al_wfrac);
  fSoilMaterial->AddMaterial(Fe,fractionmass=Fe_wfrac);
  fSoilMaterial->AddMaterial(Ca,fractionmass=Ca_wfrac);
  fSoilMaterial->AddMaterial(K,fractionmass=K_wfrac);
  fSoilMaterial->AddMaterial(Mg,fractionmass=Mg_wfrac);
  fSoilMaterial->AddMaterial(Na,fractionmass=Na_wfrac);
  fSoilMaterial->AddMaterial(Ti,fractionmass=Ti_wfrac);
  fSoilMaterial->AddMaterial(Mn,fractionmass=Mn_wfrac);
}
...
{  
  ...
  // World
  //
  G4double world_size_X = 30*cm;
  G4double world_size_Y = 30*cm;
  G4double world_size_Z = 30*cm;
  fWorldMaterial = fWoda;
  fWorldMaterial = fAirLowPressure;
  fSolidWorld = new G4Box("World", 0.5*world_size_X, 0.5*world_size_Y, 0.5*world_size_Z); 
  ...
  G4VisAttributes* visAttributes = new G4VisAttributes(G4Colour(1.0,1.0,1.0, 0.05)); // Bialy, prawie przezroczysty
...
{ 
  fWorldMaterial = nist->FindOrBuildMaterial(newMaterial);
  if (fLogicWorld != 0 ) {
...
{ 
  // Option to switch on/off checking of volumes overlaps
  G4bool checkOverlaps = true;
  // Budujemy obiekt geometryczny, w ktorym bedziemy zbierac dawke za pomoca Scorera.
  G4double innerRadius = 0*cm;
  G4double outerRadius = 5*cm;
  G4double HZ = 50*mm; //half-Z thickness
  fPhantomMaterial = fRW3;
  G4ThreeVector translation = G4ThreeVector(0, 0, -HZ);

  fSolidTank = new G4Tubs("solidTank", innerRadius, outerRadius, HZ, 0.*deg, 360.*deg);
  fLogicTank = new G4LogicalVolume(fSolidTank,  fPhantomMaterial, "logicTank"); 
  fPhysTank = new G4PVPlacement(0, translation, fLogicTank, "physTank",  fLogicWorld, false, 0, checkOverlaps); 

  G4VisAttributes* visAttributes = new G4VisAttributes(G4Colour(0.5, 0.5, 0.5, 0.5)); //szary, polprzezroczysty
  visAttributes->SetVisibility(true);
  fLogicTank->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);
}